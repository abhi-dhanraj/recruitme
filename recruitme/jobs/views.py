...
def dashboardPage(request):
    ...
    if request.session['username'] is not None:
        ...
        application_query = "select * from application_application where applicant_id_id = " + \
            str(user.id)+";"
        ...
        for i in user_applications:
            user_process_query = "select * from application_process where id = " + \
                str(i.process_id_id)+";"
            user_process = process.objects.raw(user_process_query)[0]
            print(user_process.id)
            if user_process.status == 1:
                total_selected += 1
            user_job_query = "select * from jobs_jobs where id = " + \
                str(i.job_id_id)+";"
            user_job = jobs.objects.raw(user_job_query)[0]
            user_round_query = "select * from application_rounds where id = " + \
                str(user_process.round_id_id) + ";"
        ...
        context = {'total_applied': total_applied,
                   'total_selected': total_selected, 'applications': total_applications}
